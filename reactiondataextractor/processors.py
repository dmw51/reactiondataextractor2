...
class Processor(ABC):
    ...
    def __init__(self, enabled=True, fig=None):
        ...
        self.fig = fig
        self._img = fig.img if self.fig else None
...
class ImageReader(Processor):
    ...
    def process(self):
        ...
        self.fig = Figure(img=img, raw_img=img)
        return self.fig
...
class EdgeExtractor(Processor):
    ...
    def process(self):
        if self.color_mode == self.COLOR_MODE.GRAY:
            ...
            ret, thresh = cv2.threshold(img, *config.ProcessorConfig.BIN_THRESH, cv2.THRESH_BINARY)
